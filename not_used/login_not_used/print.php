...
?>
<div class="a4">
  <h1>การแข่งขันตอบปัญหาวิทยาศาสตร์สุขภาพ<br>
  ชิงถ้วยสมเด็จพระเทพรัตนราชสุดา สยามบรมราชกุมารี<br>
  เนื่องในวันมหิดล ประจำปี พ.ศ. 2557</h1>
  <h2>Mahidol Quiz 2014</h2><br>
<?
...
$i=0;
?>  
  <div class="bold large">รายละเอียดทีม</div>
...
<?php
$sql='SELECT title, firstname, lastname, phone, school, sorted_id, exam_room FROM participant_info WHERE team_id=:id';
$stm=$db->prepare($sql);
$stm->execute(array(':id'=>$sess->ID));
while($row=$stm->fetch(PDO::FETCH_ASSOC)){
?>
<hr>
<div class="bold large">ผู้แข่งขันคนที่ <?=++$i?></div>
<table width="100%" border="0" cellspacing="2">
  <tr>
...
<div class="right small">(<?=$sess->teamName()?> | ทีม<?=($sess->type)?'อิสระ':'โรงเรียน'?>)</div>
<?php
}
}catch(Exception $e){
	echo errMsg($e,$sql);
...
try{
	$i=$_GET['i']-1;
	if($_GET['i']==0){
		$sql='SELECT sorted_id, t_firstname, t_lastname, t_phone FROM team_info WHERE id=:id LIMIT :lim, 1;';
		$i++;
	}else
		$sql='SELECT title, firstname, lastname, phone, school, sorted_id, exam_room FROM participant_info WHERE team_id=:id LIMIT :lim, 1;';
	$stm=$db->prepare($sql);
	$stm->bindParam(':id',$sess->ID);
	$stm->bindValue(':lim',$i, PDO::PARAM_INT);
	$stm->execute();
