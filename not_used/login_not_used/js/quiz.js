$(document).ready(function(e) {
	if($(":checkbox").length>0){
		$(":checkbox").click(function(e) {
			if($(":checked").length==$(":checkbox").length) $("#start").prop("disabled",false).button('enable');
			else $("#start").prop("disabled",true).button('disable');
        });
	}
    if($("#used_time").length>0){
		var startTime,updater,showTime,pg=[null,null,null];
		var endTime=1200000,countDown=10000;
		...
		showTime=setInterval(function(){
			...
			if(timer>endTime){
				...
				if(endTime-timer<34){
					...
					$("#answer").prop("readonly",false).focus();
					$("#send").prop("disabled",false).button('enable');
